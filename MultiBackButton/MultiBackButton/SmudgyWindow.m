...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.fillColor = [UIColor colorWithWhite:0.4 alpha:0.9].CGColor;
...
- (void)updateWithTouch:(UITouch *)touch {
    ...
    NSTimeInterval secs = now - self.previousTime;
    if(secs > 0.0 && self.previousTime > 0.0) {
        self.velocity = CGPointMake((self.velocity.x + (self.position.x - self.previousPosition.x) / secs) * 0.5 ,
                                    (self.velocity.y + (self.position.y - self.previousPosition.y) / secs) * 0.5 );
    }
...
- (void)disappear
{
    ...
    //if(self.velocity.x * self.velocity.x + self.velocity.y * self.velocity.y <= 100.0) {
    //    self.velocity = CGPointZero;
    //}
    //self.velocity = CGPointZero;
    ...
    self.opacity = 0.0;
    self.transform = CATransform3DMakeScale(1.5, 1.5, 1);
    self.position = CGPointMake( self.position.x + self.velocity.x * 0.1 , self.position.y + self.velocity.y * 0.1 );
